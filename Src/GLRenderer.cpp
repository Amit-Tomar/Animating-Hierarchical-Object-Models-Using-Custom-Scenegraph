...
Node * mainScene = new Scene() ;

Node * subSceneBelt1 = new Scene() ;
Node * subSceneBelt2 = new Scene() ;
Node * subSceneRobot = new Scene() ;

RobotBase robotBase;

RobotArm robotArm1(.1,1.5);
RobotArm robotArm2(.1,.5);
RobotArm robotArm3(.1,1.5);

Transform mainTransformBelt1;
Transform mainTransformBelt2;
Transform mainTransformRobot;

Transform transformRobotArm1;
Transform transformRobotArm2;
Transform transformRobotArm3;
...
void GLRenderer::timerTriggered()
{
    BeltLining *lining = new BeltLining();
    Transform *transformLiningTemp = new Transform();
    mainScene->addChild(transformLiningTemp);
    transformLiningTemp->addChild(lining);
    transformLiningTemp->setColor(.75,.75,.75);
    transformLiningTemp->interpolateTranslationTo(-2,-0.01,-2.5);
    transformLiningTemp->interpolateTranslationTo(-2,-0.01,2.5,1);

    lining = new BeltLining();
    transformLiningTemp = new Transform();
    mainScene->addChild(transformLiningTemp);
    transformLiningTemp->addChild(lining);
    transformLiningTemp->setColor(.75,.75,.75);
    transformLiningTemp->interpolateTranslationTo(1.8,-0.27,-2.5);
    transformLiningTemp->interpolateTranslationTo(1.8,-0.27,2.5,1);
